export class ModalAtributo {
        ...
        overlay.innerHTML = `
            <div class="modal-atributo">
                ...
                <div class="modal-atributo-conteudo">
                    ...
                    <!-- OPÇÕES DE VANTAGEM/DESVANTAGEM -->
                    ...
                    <!-- CONTROLE DE NÍVEL (VANTAGEM OU DESVANTAGEM) -->
                ...
                if (btnPericia) {
                    ...
                    if (desc) {
                        if (e.target.value) {
                            ...
                            if (pericia) {
                                desc.textContent = `Capacidade: ${pericia.capacidade}`;
                            }
                        } else {
    ...
    atualizarUI() {
        ...
        if (btnPericiaDesc && selectPericia && selectPericia.value) {
            const pericia = this.sistemaPericias?.pericias?.find(p => p.nome === selectPericia.value);
            if (pericia) {
                let capacidade = pericia.capacidade;
                let texto = '';
                
                if (this.tipoRolagem === 'vantagem') {
                    const total = capacidade + this.vantagemNivel;
                    texto = `Cap: ${total}d20 (${capacidade} + ${this.vantagemNivel})`;
                } else if (this.tipoRolagem === 'desvantagem') {
                    let novaQtd = Math.max(0, capacidade - this.vantagemNivel);
                    if (novaQtd === 0) {
                        texto = `Cap: 1d10 (Desvantagem extrema)`;
                    } else {
                        texto = `Cap: ${novaQtd}d20 (Desvantagem ${this.vantagemNivel}x)`;
                    }
                } else {
                    if (capacidade === 0) {
                        texto = `Cap: 1d10 (Perícia 0)`;
                    } else {
                        texto = `Cap: ${capacidade}d20`;
                    }
                }
    ...
    carregarPericias(siglaAtributo) {
        ...
        periciasDoAtributo.forEach(pericia => {
            ...
            const textoCap = cap === 0 ? 'Cap: 0 (1d10)' : `Cap: ${cap}`;
    ...
    rolarComPericia(nomePericia) {
        ...
        if (this.tipoRolagem === 'desvantagem') {
            capacidade = Math.max(0, capacidade - this.vantagemNivel);
        }
        ...
        if (this.sistemaDados.rolarTesteAtributo) {
            ...
            this.sistemaDados.rolarTesteAtributo(nome, capacidade);
        }
    }

export class BarraVital {
    ...
    atualizarBarra() {
        if (this.valorMaximo === 0) {
            this.barraPreenchimento.style.width = '0%';
            this.removerEfeitos();
            return;
        }
        
        const porcentagem = Math.min(Math.max(this.valorAtual / this.valorMaximo * 100, 0), 100);
    ...
    setValorAtual(valor, naoDispararEvento = false) {
        this.valorAtual = Math.min(Math.max(valor, 0), this.valorMaximo);
        this.atualizarValores();
        this.atualizarBarra();
    ...
    setValores(atual, maximo, calculoAutomatico = false) {
        ...
        const novoAtual = Math.min(Math.max(parseInt(atual) || 0, 0), this.valorMaximo);

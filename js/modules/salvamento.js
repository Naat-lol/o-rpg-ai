(function() {
    ...
    function coletarDados() {
        ...
        const infoGerais = {};
        const nomeInput = document.querySelector('.dados-basicos input[placeholder="Nome do personagem"]');
        if (nomeInput) infoGerais.nomePersonagem = nomeInput.value;
        const jogadorInput = document.querySelector('.dados-basicos input[placeholder="Nome do jogador"]');
        if (jogadorInput) infoGerais.nomeJogador = jogadorInput.value;
        const idadeInput = document.querySelector('.dados-basicos input[placeholder="Idade"]');
        if (idadeInput) infoGerais.idade = idadeInput.value;
        const generoInput = document.querySelector('.dados-basicos input[placeholder="Gênero"]');
        if (generoInput) infoGerais.genero = generoInput.value;
        const trilhaInput = document.getElementById('trilha-principal');
        if (trilhaInput) infoGerais.trilhaPrincipal = trilhaInput.textContent;
        ...
        if (fotoImg && fotoImg.src && !fotoImg.src.includes('via.placeholder.com') && !fotoImg.src.includes('data:image/svg')) {
    ...
    function restaurarDados(dados) {
        ...
        if (dados.pericias && window.sistemaPericias) {
            ...
            window.sistemaPericias.atualizarListaPericias();
        ...
        if (dados.infoGerais) {
            ...
            if (movimentoInput && dados.infoGerais.movimento !== undefined) {
                ...
                movimentoInput.dispatchEvent(new Event('change', { bubbles: true }));
            }
            const defesaInput = document.querySelector('.campo-md:last-child input');
            if (defesaInput && dados.infoGerais.defesa !== undefined) {
                defesaInput.value = dados.infoGerais.defesa;
                defesaInput.dispatchEvent(new Event('change', { bubbles: true }));
            }
        }
    ...
    function resetarTodosOsSistemas() {
        ...
        document.querySelectorAll('.dados-basicos input[placeholder="Nome do personagem"]').forEach(el => el.value = '');
        document.querySelectorAll('.dados-basicos input[placeholder="Nome do jogador"]').forEach(el => el.value = '');
        document.querySelectorAll('.dados-basicos input[placeholder="Idade"]').forEach(el => el.value = '');
        document.querySelectorAll('.dados-basicos input[placeholder="Gênero"]').forEach(el => el.value = '');
    ...
    function baixarFicha() {
        try {
            ...
            link.href = url;
            const nomePersonagem = document.querySelector('.dados-basicos input[placeholder="Nome do personagem"]')?.value || 'ficha';
    ...
    function configurarEventosAutomaticos() {
        document.addEventListener('input', (e) => {
            ...
            if (e.target.closest('.dados-basicos') || e.target.closest('.campo-md')) {
                agendarSalvamento();
            }
        });

    ...
    constructor() {
        ...
        this.catalogoPerks = [
            ...
            },
            {
                ...
                custo: 0,
                necessita: null,
                modificacao: { campo: 'movimento', valor: 3, tipo: 'fixo' }
            },
            {
                ...
                custo: 0,
                necessita: null,
                modificacao: { campo: 'defesa', valor: 2, tipo: 'fixo' }
            },
            {
            ...
            },
            {
                ...
                custo: 0,
                necessita: null,
                modificacao: { campo: 'vidaMax', valor: 2, tipo: 'porDezenaEXP' }
            },
            {
                id: 'perk-equilibrio-mental',
                nome: 'Equilíbrio Mental',
                descricao: 'Você recebe +10 de Sanidade máxima para cada 10% de Exposição adquirida, fortalecendo sua mente contra os horrores encontrados ao longo da jornada.',
                custo: 0,
                necessita: null,
                modificacao: { campo: 'sanidadeMax', valor: 10, tipo: 'porDezenaEXP' }
            },
            {
                id: 'perk-determinado',
                nome: 'Determinado',
                descricao: 'Você recebe +5 de Desejo (PD) máximo para cada 10% de Exposição adquirida, aumentando sua determinação e força de vontade.',
                custo: 0,
                necessita: null,
                modificacao: { campo: 'desejoMax', valor: 5, tipo: 'porDezenaEXP' }
            },
            {
        ...
        this.sistemaBarras = null;
        this.sistemaTamanho = null;

        this.inicializarUI();
        this.configurarEventosRecalculo();
    }

    configurarEventosRecalculo() {
        document.addEventListener('atributoAlterado', (e) => {
            const { atributo, valor } = e.detail;
            if (['FOR', 'DES', 'TAM', 'EXP'].includes(atributo)) {
                this.recalcularBonus();
            }
        });

        document.addEventListener('movimentoDefesaBaseAtualizado', () => {
            this.recalcularBonus();
        });
    }

    setSistemas(sistemaBarras, sistemaTamanho) {
        this.sistemaBarras = sistemaBarras;
        this.sistemaTamanho = sistemaTamanho;
        this.recalcularBonus();
    }

    recalcularBonus() {
        if (!this.sistemaBarras || !this.sistemaTamanho) return;

        const { vida, sanidade, desejo } = this.sistemaBarras;
        const tamanho = this.sistemaTamanho;

        const movimentoBase = tamanho.getMovimentoBase();
        const defesaBase = tamanho.getDefesaBase();

        const expInput = document.querySelector('.atributo-item .input-porcentagem input[type="number"]');
        const exp = expInput ? parseInt(expInput.value) || 0 : 0;
        const dezenasEXP = Math.floor(exp / 10);

        let bonusMovimento = 0;
        let bonusDefesa = 0;
        let bonusVidaMax = 0;
        let bonusSanidadeMax = 0;
        let bonusDesejoMax = 0;

        this.perksSelecionadas.forEach(perk => {
            if (!perk.modificacao) return;
            const { campo, valor, tipo } = perk.modificacao;

            let valorEfetivo = 0;
            if (tipo === 'fixo') {
                valorEfetivo = valor;
            } else if (tipo === 'porDezenaEXP') {
                valorEfetivo = valor * dezenasEXP;
            }

            switch (campo) {
                case 'movimento': bonusMovimento += valorEfetivo; break;
                case 'defesa': bonusDefesa += valorEfetivo; break;
                case 'vidaMax': bonusVidaMax += valorEfetivo; break;
                case 'sanidadeMax': bonusSanidadeMax += valorEfetivo; break;
                case 'desejoMax': bonusDesejoMax += valorEfetivo; break;
            }
        });

        const campoMovimento = document.querySelector('.movimento-defesa .campo-md:nth-child(1) input');
        const campoDefesa = document.querySelector('.movimento-defesa .campo-md:nth-child(2) input');
        if (campoMovimento) {
            campoMovimento.value = movimentoBase + bonusMovimento;
        }
        if (campoDefesa) {
            campoDefesa.value = defesaBase + bonusDefesa;
        }

        if (vida) vida.setBonusMaximo(bonusVidaMax);
        if (sanidade) sanidade.setBonusMaximo(bonusSanidadeMax);
        if (desejo) desejo.setBonusMaximo(bonusDesejoMax);
    }

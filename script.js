...
function atualizarEscurecimentoBarra(tipo) {
    const preenchimento = tipo === 'vida' ? 
        ...
        (tipo === 'sanidade' ? 
            ...
            document.getElementById('barra-desejo-preenchimento'));
    
    const max = tipo === 'vida' ? vidaMaxima : 
                (tipo === 'sanidade' ? sanidadeMaxima : desejoMaxima);
    const atual = tipo === 'vida' ? vidaAtual : 
                  (tipo === 'sanidade' ? sanidadeAtual : desejoAtual);
    ...
    if (tipo === 'vida') {
        preenchimento.style.backgroundColor = '#FF3838'; // Vermelho para vida
    } else if (tipo === 'sanidade') {
        preenchimento.style.backgroundColor = '#9D00FF'; // Roxo para sanidade
    } else if (tipo === 'desejo') {
        preenchimento.style.backgroundColor = '#3BA18A'; // Amarelo para desejo
    }
}
...
function atualizarBarra(tipo) {
    ...
    let valorAtual = tipo === 'vida' ? vidaAtual : 
                    (tipo === 'sanidade' ? sanidadeAtual : desejoAtual);
    
    let valorMax = parseInt(maxInput.value) || 0;
    const maxNegativo = (tipo === 'vida') ? MAX_NEGATIVO_VIDA : 
                       (tipo === 'sanidade' ? MAX_NEGATIVO_SANIDADE : 0); // Desejo não tem negativo
...
function mudarStatus(tipo, quantidade) {
    ...
    if (tipo === 'vida') {
        ...
        desejoAtual += quantidade;
        
        desejoAtual = Math.min(desejoAtual, desejoMaxima); 
        desejoAtual = Math.max(desejoAtual, maxNegativo); // maxNegativo = 0 para desejo
        
        atualizarBarra('desejo');
    }
...
function salvarDados() {
    const dados = {
        ...
        corrupcao: document.getElementById('atributo-vinculo')?.value || '',
...
function carregarDados() {
    ...
    if (dadosSalvos) {
        ...
        console.log("Desejo salva:", dadosSalvos.desejoAtual, "/", dadosSalvos.desejoMax);
        ...
        const desejoMaxHTML = parseInt(document.getElementById('desejo-Max').value);
        ...
        desejoMaxima = parseInt(dadosSalvos.desejoMax) || desejoMaxHTML || 10;
        ...
        document.getElementById('desejo-Max').value = desejoMaxima;
        ...
        desejoAtual = (dadosSalvos.desejoAtual !== null && dadosSalvos.desejoAtual !== undefined) 
            ? parseInt(dadosSalvos.desejoAtual) 
            : desejoMaxima;
...
setTimeout(() => {
    aplicarEfeitosVisuais();
        ...
        document.getElementById('atributo-vinculo').value = dadosSalvos.corrupcao || '0';
...
atualizarListaInventario();
    ...
    atualizarEscurecimentoBarra('desejo');
...
function configurarEventListeners() {
    ...
    document.getElementById('desejo-10').addEventListener('click', () => mudarStatus('desejo', -10));
    document.getElementById('desejo-1').addEventListener('click', () => mudarStatus('desejo', -1));
    document.getElementById('desejo+1').addEventListener('click', () => mudarStatus('desejo', 1));
    document.getElementById('desejo+10').addEventListener('click', () => mudarStatus('desejo', 10));
...
document.getElementById('sanidade-Max').addEventListener('input', (event) => {
    ...
    document.getElementById('desejo-Max').addEventListener('input', (event) => {
    desejoMaxima = parseInt(event.target.value) || 0;
    if (desejoAtual > desejoMaxima) {
        desejoAtual = desejoMaxima;
    }
    atualizarBarra('desejo');
    salvarDados();
